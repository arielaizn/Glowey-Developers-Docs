---
title: Lipsync
description: Synchronize audio to avatar and video faces for lifelike character animations.
---
...
Fixed per model. See [Credits & Pricing](/docs/credits-and-pricing#lipsync).
...
```json
{
  ...
  "taskId": "task_lip789def",
  "output": {
    "video_url": "https://storage.gloweyai.com/lipsync_xyz789.mp4"
  }
}
```
...
```bash
curl "https://gloweyai.com/api/lipsync/status?taskId=task_lip789def&modelId=sync-lipsync-v2" \
  -H "Authorization: Bearer glow_sk_YOUR_TOKEN_HERE"
```
...
- **Audio Format:** MP3, WAV, or M4A. Duration: less than 1 minute (most models)
- **Video Format:** MP4, MOV, or GIF. Size: less than 50 MB
